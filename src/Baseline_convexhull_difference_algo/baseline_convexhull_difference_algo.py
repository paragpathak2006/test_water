...
def baseline_convexhull_difference(solid_volume : Trimesh):
    ...
    max_volume_index = max(range(len(fluid_volumes)), key=lambda i: fluid_volumes[i].volume)
    i = max_volume_index

    print("\nLargest fluid volume is volume#", max_volume_index, " with volume = ", fluid_volumes[max_volume_index].volume)
    ...
    fluid_boundary["difference"].export(OUT_DIR / f"3. fluid_inlets-outlets-combined-{i}.stl")
